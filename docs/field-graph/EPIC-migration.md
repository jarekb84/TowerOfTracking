...
- **Phase 2 (commits 4–14)** is the vertical-slice sequence. Each commit declares one edge type, wires the graph into the consumers that need it, and deletes the imperative code it replaces. App behavior stays equivalent through the phase; only the mechanism changes. **Commits 4 and 5 surfaced two architectural concerns**: (a) `FieldGraph`'s API surface is heading toward a god object, and (b) cutovers risked landing as variable-swaps without eliminating the conditional logic the graph is supposed to subsume (csv-exporter ladder is the canonical example). **Commits 5b (API ergonomics + `Node.tags` removal) and 5c (field-aware export extractors) gate commits 6+** until both are resolved — no more vertical slices land while the architecture is in transition.
...
- [ ] **Commit 5b — Field-graph API ergonomics review + `Node.tags` removal**
  - **Why (API surface):** Commits 4 + 5 added `acceptedValuesFor`, `isAcceptedValue`, `matchAcceptedValue`, `enumValueMeta`, `enumValuesOf`, `csvHeaderOf`, `internalFields`, `isInternalField` plus the polymorphic-input handling. Each subsequent edge type wants to add 2–4 more accessor methods (commit 6: `rendersAsIn`; commit 8: `dataTypeOf`; commit 9: `fieldsDerivedFrom`/`derivationsOf`; commit 11: `currentSchema`/`schemaOf`/`fieldsShippedIn`/`fieldsDroppedIn`; commit 12: `fieldsInView`/`fieldsInFilter`/`viewsOf`/`filtersOf`; commit 13: `conditionalOn`/`conditionallyVisibleFields`; commit 14: `isRequiredIn`/`requiredFieldsIn`/`participatesInCompositeKey`/`compositeKeyFieldsFor`). Continuing this trajectory yields ≥25 methods on `FieldGraph` by commit 15. The "engine-method-per-consumer-pattern" rule from commit 4 was the right call for that commit, but at this scale the engine class becomes a god object and the file's load-bearing — every edge touches it.
  - **Why (`Node.tags`):** Commit 5 declared `IS_INTERNAL_FIELD` edges for the same five fields that already carried `tags: ['internal']`. The redundancy was tracked in [`EXPLORATION-tag-vs-edge.md`](./EXPLORATION-tag-vs-edge.md), and the decision (per the author) is: **flat-out remove the `tags` concept.** Edges are structurally queryable; tags are unstructured node-local strings. At our scale, every fact a consumer queries should be an edge — tags would be a parallel, less-powerful representation. Commit 4's locked design carries `Node.tags` in `types.ts`; this commit removes it.
  - **Scope (API ergonomics — decision-shaped):** Pick ONE of the following directions for the engine API. Land the choice as a short ADR and one round of refactoring on the existing methods.
    - **(A) Per-edge query modules.** Each `catalog/*.edges.ts` file ships a sibling `*.queries.ts` that exports plain functions taking a `FieldGraph` (or just the indexed maps). `FieldGraph` exposes a small primitive surface (`edgesFrom`, `edgesTo`, `nodesOfKind`, `terminalOf`); domain queries live next to their declarations. Tradeoff: more files; consumer imports get longer (`import { csvHeaderOf } from '@/shared/domain/field-graph/catalog/internal-fields.queries'`).
    - **(B) Sub-API namespaces on `FieldGraph`.** `graph.enums.acceptedValuesFor(...)`, `graph.internal.csvHeaderOf(...)`, `graph.sections.fieldsInSection(...)`. One class, multiple namespaced sub-objects assembled in the constructor. Tradeoff: keeps the singleton; some plumbing per namespace.
    - **(C) Plugin / registry.** Each edge file calls `registerQueryMethods({ csvHeaderOf, internalFields, isInternalField })` at module load; `FieldGraph` indexes them. Tradeoff: dynamic surface — TypeScript can't autocomplete from a flat `graph.X` interface; harder to discover.
    - **(D) Status quo + lint cap.** Stay on the flat method-per-consumer surface but cap method count at, say, 30. Tradeoff: kicks the can; review burden grows linearly with edges.
  - **Scope (`Node.tags` removal — implementation-shaped):**
    1. Delete `tags?: readonly string[]` from `Node` in [`types.ts`](../../src/shared/domain/field-graph/types.ts).
    2. Delete the `tags` parameter / option from `fieldNode()` and other node-builders in [`builders.ts`](../../src/shared/domain/field-graph/builders.ts).
    3. Delete `tags: ['internal']` from the five internal-field declarations in [`fields.nodes.ts`](../../src/shared/domain/field-graph/catalog/fields.nodes.ts) — `IS_INTERNAL_FIELD` edges are the source of truth.
    ...
    6. Future-proofing: planned `'tournament-only'` tag (Notes-and-findings entry from commit 3) is dropped — the `_RANK_NODE CONDITIONAL_ON ENUM_RUN_TYPE__TOURNAMENT_NODE` edge in commit 13 covers it. Planned `'user-text'` and `'nullable-empty-string'` tags from spec §11.4 are absorbed by edges in commits 8 / 14 — captured in those commits' "Cutover requirement" lines when their scopes are next refined.
  - **DoD:** API ADR landed in `docs/field-graph/`. Existing methods refactored to fit the chosen shape (or an explicit decision to defer the refactor to a tail-end commit captured in the ADR). `Node.tags` deleted; no node declaration carries a `tags` option; `fields.nodes.test.ts` queries the graph for internal-field membership. `npm run integration-precheck` green.
  - **Dependencies:** commits 4, 5 (need real-world consumer surface and the IS_INTERNAL_FIELD edge to evaluate against / replace tag with).
  - **Status:** `TODO`
  - **Note:** Commits 6+ are paused until both 5b and 5c land.

- [ ] **Commit 5c — Field-aware export extractors (eliminate per-internal-field switch ladders)**
  - **Why:** Commit 5 cut over `INTERNAL_FIELD_NAMES` to use `_X_NODE.id`, but it left the per-internal-field `if/else` ladder in [`csv-exporter.ts`](../../src/features/data-export/csv-export/csv-exporter.ts) intact — five branches each with bespoke fallback logic (`run.fields[_DATE_NODE.id]?.rawValue || formatIsoDate(run.timestamp)`, `encodeNotesForStorage(...)`, `|| run.runType`, `|| ''`, etc.). The whole point of the graph architecture is to eliminate this kind of conditional logic. **Variable-swap is not cutover.** A vertical slice that ports the field IDs but leaves the per-field switch is the letter, not the spirit, of the architecture.
  - **Scope:** Introduce a new edge `HAS_CSV_EXTRACTOR` (terminal-string target — names a registered function) and a small registry `CSV_EXTRACTORS: Record<string, (run: ParsedGameRun) => string>` co-located with the catalog. Declare one extractor per internal field encoding the field-specific raw-value-read + fallback. Add engine helper `csvExtractorOf(fieldRef): string | undefined`. Rewrite the csv-exporter loop so it reads `value = CSV_EXTRACTORS[graph.csvExtractorOf(field) ?? 'default'](run)` — no per-field branches.
  - **Spec rationale:** Same architectural pattern as commit 9's `IS_DERIVED_FROM` + `DERIVERS` registry (architecture/11-internal-app-fields.md §11.3), applied to *export-time* extraction instead of *parse-time* derivation. Same shape: edge declares the deriver name; registry maps name → function; engine helper resolves; loop applies. Commit 9 will likely consolidate both registries (input direction and output direction) into one — flagged as a follow-up there.
  - **Files touched:** [`csv-exporter.ts`](../../src/features/data-export/csv-export/csv-exporter.ts) (delete both ladders, replace with a one-line extractor call). New: `catalog/csv-extractors.ts` (or co-located in `catalog/internal-fields.edges.ts` if small). [`types.ts`](../../src/shared/domain/field-graph/types.ts) gets `HAS_CSV_EXTRACTOR` added to `EdgeType` + `EDGE_META`. Engine gets `csvExtractorOf()`.
  - **Cutover requirement (per preamble §5):** ZERO per-internal-field `if (fieldName === _X_NODE.id)` branches survive in `csv-exporter.ts`. Audit other consumers for similar export-time switch ladders (none surfaced in commit 5's audit beyond the two ladders inside the exporter, but recheck — the trip wire is "switch on internal-field id"). After this commit lands, "what's this field's CSV value for this run?" is answered by one graph-driven function call, not a switch.
  - **Conventions:** `csvExtractorOf` follows the engine-method-per-consumer rule (subject to whatever shape commit 5b picks). The `CSV_EXTRACTORS` registry mirrors commit 9's `DERIVERS` shape.
  - **DoD:** CSV round-trip tests pass; the `if/else` ladder in `csv-exporter.ts` is gone (replaced with a single call); a new test asserts that adding a `HAS_CSV_EXTRACTOR` edge to a fixture field auto-includes its export logic without touching the exporter code.
  - **Dependencies:** commit 5 (IS_INTERNAL_FIELD + HAS_CSV_HEADER), commit 5b (API shape decided — affects how `csvExtractorOf` is declared).
  - **Status:** `TODO`
  - **Note:** This commit also clarifies a class of mistakes the prompt skeleton now flags: "swapping field-name constants ≠ cutover; conditional logic that the graph should eliminate is the real architectural smell." See [`prompt-skeleton.md`](./prompt-skeleton.md) §self-check.
...
- [ ] **Commit 6 — `BELONGS_TO_SECTION` + `RENDERS_AS_IN_SECTION` edges**
  ...
  - **Dependencies:** commits 1–3, **5b** (API ergonomics + tags removal), **5c** (field-aware export extractors).
  - **Status:** `TODO`
...
- [ ] **Commit 8 — `HAS_DATA_TYPE` edges + parser type-detector rewrite**
  ...
  - **Cutover requirement (per preamble §5):** every label-pattern-matching call site for data-type detection moves to `graph.dataTypeOf(fieldRef)`. The label-based heuristic in `field-utils.ts` is deleted (or reduced to a graph-query one-liner). Audit all `field.label.includes(...)` / `field.dataType` derivation sites and migrate them.
  - **Conventions (per preamble §10):** lift `DATA_TYPES = ['number', 'duration', 'date', 'string'] as const` to a TS source-of-truth file (`shared/domain/field-graph/data-types.ts` or similar). Add engine helper `dataTypeOf(fieldRef): DataType | undefined`. Edge declarations reference `<NAME>_NODE.id` per the named-export rule.
  - **DoD:** `v28-sample-parse.invariant.test.ts` still passes; add a new invariant that every field node has exactly one `HAS_DATA_TYPE` edge. No call site still derives data type by label-matching.
...
- [ ] **Commit 9 — `IS_DERIVED_FROM` edges + derivation cascade**
  ...
  - **Cutover requirement (per preamble §5):** the inline `deriveDateTimeFromBattleDate(battleDate)` calls in `data-parser.ts` and `csv-parser.ts` (currently three call sites — one per parser, one in form updates) are replaced by a single `applyDerivations(fields)` call. Same for any other ad-hoc derivation logic in form update or import paths. After the commit, no parser hardcodes the `_date` / `_time` derivation by name.
