...
- [ ] **Commit 6 — `BELONGS_TO_SECTION` + `RENDERS_AS_IN_SECTION` edges**
  - **Scope:** Declare section membership for every field. Rewrite [`section-config.ts`](../../src/features/game-runs/card-view/run-details/section-config.ts) to query the graph — `BATTLE_REPORT_ESSENTIAL`, `DAMAGE_TAKEN_CONFIG`, etc. become graph queries. `RENDERS_AS_IN_SECTION` handles per-section display overrides (e.g. `battleReport_cellsEarned` showing under both battleReport and currencies). [`section-config.ts`](../../src/features/game-runs/card-view/run-details/section-config.ts) carries a transitional comment explicitly pointing at this commit; that file is deleted here.
  ...
  - **Files touched:** [`section-config.ts`](../../src/features/game-runs/card-view/run-details/section-config.ts) (deleted), `use-run-details-data.ts`, plus a few chart pages that reference section configs. New: `catalog/edges/sections/sections.edges.ts` (~150 entries).
