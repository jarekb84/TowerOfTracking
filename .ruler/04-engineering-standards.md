...
*See 06-react-separation.md for complete logic-presentation separation standards, testing requirements, and the three-layer architecture (*.tsx → use*.ts → *.ts).*

**Key Points (Quick Reference)**:
- Components (*.tsx): DOM/JSX only, 200-line hard limit
- Hooks (use*.ts): State/effects orchestration, delegate logic to pure functions
