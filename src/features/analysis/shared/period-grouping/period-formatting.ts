 ...
 *
 * These utilities are shared across analysis features (source-analysis,
 * coverage-report) that need consistent period key generation and label
...
/**
 * Format hourly period key (YYYY-MM-DDTHH) to display label
 * Uses locale-aware formatters for both date and hour portions.
 * e.g., "Mar 15 2 PM" (en-US) or "15. Marz 14" (de-DE)
 */
function formatHourlyLabel(key: string): string {
  const [datePart, hourStr] = key.split('T');
  const [year, month, day] = datePart.split('-');
  const hour = Number(hourStr);
  const date = new Date(Number(year), Number(month) - 1, Number(day), hour);
  const monthDay = formatDisplayMonthDay(date);
  const hourLabel = formatDisplayHour(date);
 ...
 * Shows month abbreviation and 2-digit year (e.g., "Mar '24" or "Marz '24")
 */
